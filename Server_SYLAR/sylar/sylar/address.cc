...
static const size_t MAX_PATH_LEN = sizeof(((sockaddr_un*)0)->sun_paht) -1;

UnixAddress::UnixAddress(){
    memeset(&m_addr,0,sizeof(m_addr));
...
std::ostream& UnixAddress::insert(std::ostream& os) const{
    if(m_length > offsetof(sockaddr_un,sun_path)
        ...
        return os << "\\0" << std::string(m_addr.sun_path + 1,
                m_length - offset(sockaddr_un,sun_path) -1);
    }
    return os << m_sun.sun_path;
}
